...
Implementation Plan artifact must include: the first template's concept (mood, layout, one signature animation), the schema, the route and endpoint list, the auth/cookie/CSRF design, the revision scheme, the test plan mapped to Gates A-D, and owner decisions O1-O2.

## Build order
1. Schema (Prisma): `Experience` with `publicId`, `editCredentialHash`, `editCredentialVersion`, `editCredentialIssuedAt`, `templateId`, `templateVersion`, `draftConfig`, `draftRevision`, `publishedConfig?`, `publishedRevision?`, `publishedAt?`, `status` (`draft | published | disabled | deleted`), timestamps. Non-destructive migration.
...
5. Editor at `/edit/[publicId]`: form, live preview (stacked on mobile), autosave + Save Draft with status indicator, reminder banner for browser-stored edit access, Publish with strict-validation errors, copy-URL with fallback.
6. Public page `/v/[publicId]`: `publishedConfig` only, mobile-first, noindex, no-store, and generic metadata, 404/410 states. Handlers ignore edit cookies completely.
7. Tests for Gates A-D, then `browser-verify`, then `security-reviewer` and `code-reviewer`.

## Gate
All four acceptance gates in `docs/m1-decisions.md` section 10 pass, plus `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:e2e`, `pnpm build`. Promote the decisions per section 12. Checkpoint report via `phase-runner`.
